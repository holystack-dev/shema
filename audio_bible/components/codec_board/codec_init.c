...
static int _i2c_init(uint8_t port)
{
    ...
    // Reuse a bus already created elsewhere (e.g. by i2c_bsp) on this port
    // instead of failing with "I2C bus already acquired".
    {
        i2c_master_bus_handle_t existing = NULL;
        if (i2c_master_get_bus_handle(port, &existing) == ESP_OK && existing) {
            i2c_bus_handle[port] = existing;
            i2c_inited[port] = true;
            ESP_LOGI(TAG, "Reusing existing I2C bus on port %d", port);
            return 0;
        }
    }
