...
void adc_bsp_init(void)
{
  ...
  ESP_ERROR_CHECK(adc_oneshot_config_channel(adc1_handle, ADC_CHANNEL_7, &config));  // batt sense (GPIO8) on 1.85C V2
}
...
#endif
  esp_err_t err;
  err = adc_oneshot_read(adc1_handle,ADC_CHANNEL_7,&adcdata);
  if(err == ESP_OK)
  {
#ifdef ADC_Calibrate
...
void adc_example(void* parmeter)
{
  ...
  for(;;)
  {
    ...
    ESP_LOGI("adc","adc value:%d,system voltage:%f",adcdata,_vol);
